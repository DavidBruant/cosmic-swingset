...
async function buildSwingset(
  ...
  for (const fname of fs.readdirSync(vatsDir)) {
    ...
    if (match) {
      config.vats.set(match[1], { sourcepath: require.resolve(`${vatsDir}/${fname}`) });
    }
  }
  config.vats.set('vattp', {sourcepath: getVatTPSourcePath() });
  config.vats.set('comms', {sourcepath: getCommsSourcePath(),
...
export async function launch(mailboxStorage, stateFile, vatsDir, argv) {
  ...
  try {
    ...
    console.log(
      `launch: found saved swingset state, size= ${initialState.length}`,
    );
  } catch (e) {
