...
function build(E, log) {
  ...
  async function startup(host) {
    ...
    const seat = await E(host).redeem(playerInvite)
    
    const guess = await E(seat).guess

    console.log('ZZZ guess', typeof guess, guess)

    const result = await guess(37)
    console.log('ZZZ result', typeof result, result)
